...
Public Sub Main()
    ...
    Dim strWin As String, strHost As String, strCDltr As String
    Dim ExitCode As String, oWShell As Object
    ...
    strProgram = Chr(34) & "msiexec /i " & strCDltr & "\Streets\MSMap\data.msi TRANSFORMS=" & strCDltr & "\Streets\MSMap\data.mst /qn" & Chr(34)
    'strWorkingDir = strWin & "\system32\"
    strWorkingDir = strCDltr & "\Streets\MSMap\"
    RunAsUser strUser, strPassword, strHost, strProgram, strWorkingDir
End Sub

Public Function FindCDRom()
    Dim drv As Object, tagfile As String, oFSO As Object, strCDR As String
    tagfile = "\Setup_ST.exe"
    Set oFSO = CreateObject("Scripting.FileSystemObject")
    For Each drv In oFSO.Drives
        Dim strDriveLtr
        If drv.DriveType = 4 Then
            strDriveLtr = drv.DriveLetter
            If oFSO.FileExists(strDriveLtr & ":" & tagfile) Then strCDR = drv.DriveLetter
        End If
    Next
    strCDR = strCDR & ":"
    FindCDRom = strCDR
